...
## 你要做的事
研究兩條利基資料線，找出未來約 8 週內、**免費**且適合帶幼兒參加的活動，更新 repo 內 JSON 並提交。
只收錄免費活動（`isFree` 必為 true）；需購票或付費才能參加的活動一律不收錄。
「岸上觀賽／自由入場免費，但另有付費選項（如報名比賽、付費船班）」這類仍算免費，於 `priceNote` 註明免費範圍即可。
...
所有收錄活動都已是免費，以下僅以「場域形式／適合年齡」區分，與票價無關：
- great：戶外/開放空間、可隨進隨出、容忍幼兒走動吵鬧、時間不長、推車友善、週末或白天。
- ok：室內或稍有形式、時間夠短、家長陪同下 2–4 歲尚可、可能需報名／索票。
- older：需久坐安靜、時間長、對象學齡以上。仍收錄但標清楚。
...
2. 逐一研究來源（用 web 搜尋/瀏覽）。每找到一筆活動，比對是否已存在（以 sourceName|title|startDate 去重）：
   ...
   - 已存在但有更新（如日期確定、報名／索票方式變動）→ 更新該筆欄位。
...
- 只收錄免費活動；無法確認免費、或需購票付費者一律不收，不要為了湊數而放入付費活動。
